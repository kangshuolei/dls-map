...
Shaders.prototype = {
  ...
  // 动态线
  _getDynamicLineShader: function (options: any) {
  ...
  // 动态泛光线
  _getDynamicLightLineShader: function (options: any) {
    if (options && options.get) {
      return 'czm_material czm_getMaterial(czm_materialInput materialInput)\n\
              {\n\
                  czm_material material = czm_getDefaultMaterial(materialInput);\n\
  ...
  //带方向的墙体
  _getDirectionWallShader: function (options: any) {
    if (options && options.get) {
      var materail =
        'czm_material czm_getMaterial(czm_materialInput materialInput)\n\
                  {\n\
  ...
  _getCircleFadeShader: function (options: any) {
    if (options && options.get) {
      return `czm_material czm_getMaterial(czm_materialInput materialInput)\n                
                  {\n                    
                      czm_material material = czm_getDefaultMaterial(materialInput);\n                    
  ...
  // 波动圆
  _getDynamicCircleShader: function (options: any) {
    if (options && options.get) {
      return 'uniform vec4 color;\n\
              uniform float duration;\n\
              uniform float count;\n\
  ...
  // 雷达扫描
  _getRadarScanShader: function (options: any) {
    if (options && options.get) {
      return (
  ...
  // 圆形扫描
  _getCircleScanShader: function (options: any) {
    if (options && options.get) {
      return (
        'uniform sampler2D colorTexture;\n\
              uniform sampler2D depthTexture;\n\
